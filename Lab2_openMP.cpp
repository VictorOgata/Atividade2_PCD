...
int main(){
    ...
    int CellsV[MAX_THREADS];
    #pragma omp parallel default(none) shared(Grid,CellsV)
    {
        CellsV[omp_get_thread_num()]=0;
        for(int a = (GRID_SIZ/MAX_THREADS)*omp_get_thread_num(); a < GRID_SIZ/MAX_THREADS*(omp_get_thread_num()+1); a++){
            for(int b = 0; b < GRID_SIZ; b++){
                if (Grid[(a*GRID_SIZ)+b])
                    CellsV[omp_get_thread_num()] += 1;
            }
        }
    }
    int Cells = 0;
